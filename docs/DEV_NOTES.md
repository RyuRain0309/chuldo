...
- **참가자 목록에서 스크롤 밖(화면에 안 보이는) 인원이 안 잡힘**
  ...
  2. 창에 `WM_MOUSEWHEEL` 메시지를 직접 전달(`PostMessage`, win32 API) — 커서는 안
     ...
     느렸음(참가자 많으면 15초 안팎). 이후 "스크롤 전/후 겹치는 인원 있으면 그 사이는
     안 건너뛴 게 보장"이라는 원리로 스텝 크기를 적응형으로 키워봤지만 그래도 노치
     단위라 한계가 있었음
  3. **최종: Page Down 키(`WM_KEYDOWN`/`WM_KEYUP`, `VK_NEXT`)도 `PostMessage`로 전달**
     (`automation/scraper.py`의 `post_key`/`collect_participants`). `SetFocus`를 안
     부르므로 창이 활성화(포커스 뺏김)되지 않는 것까지 실측으로 확인함. Page Down은
     관례적으로 한 번에 거의 한 페이지만큼 스크롤하면서 마지막 줄 일부를 다음 페이지에도
     겹쳐 보여주는 게 표준 동작이라, 노치 크기를 추측/보정할 필요가 없어 휠 방식보다
     훨씬 빠름. "겹치면 안전" 원리는 그대로 유지해서 겹침 0이 감지되면 로그만 남김.
